...
from utils import (
    ...
    print_kaplan_mayers,
    regression,
    roc, load_and_prepare, nemenyi_friedmann, print_kaplan_mayers_together
)
...
print_kaplan_mayers_together(ronaldo, messi)
...
for thresh in [0.25, 0.75, 0.5]:
    tprs, aucs, mean_fpr, accuracies, precisions, recalls, balanced_accuracies = regression(df, thresh, random_state=random_state)
    print(f"Średnia dokładność dla threshold {thresh}: {np.mean(accuracies):.4f} ± {np.std(accuracies):.4f}")
    print(f"Średnia zbalansowana dokładność dla threshold {thresh}: {np.mean(balanced_accuracies):.4f} ± {np.std(balanced_accuracies):.4f}")
    print(f"Średnia precyzja dla threshold {thresh}: {np.mean(precisions):.4f} ± {np.std(precisions):.4f}")
    print(f"Średnia czułość (recall) dla threshold {thresh}: {np.mean(recalls):.4f} ± {np.std(recalls):.4f}")
    print('==============================================================')
